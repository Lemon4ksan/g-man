...
Этот каталог содержит общедоступные пакеты G-man. Вы можете импортировать фреймворк целиком или точечно выбирать отдельные пакеты (например, `steam/community` для скрейпинга, `trading/engine` для создания конвейеров проверок или `behavior/processor` для аудита сделок) для интеграции в существующие проекты.
...
flowchart TD
    ...
    subgraph L4 ["🚀 Слой 4: Домен и исполнение (Бизнес-логика)"]
        ...
        Behavior["<b>🤖 behavior</b><br/>Автономные сценарии<br/>Достижения, Процессор и Guard"]
    ...
    subgraph L1 ["🛠️ Слой 1: Инфраструктурные утилиты"]
        direction LR
        Foundation["<b>📊 foundation / miyako</b><br/>Логирование и Асинхронность"]
        InternalNet["<b>🌐 internal/network</b><br/>Базовый TCP/WS транспорт"]
...
### 1. Базовый слой и Protobuf (`pkg/steam` & `protobuf`)
...
| **[protobuf](../protobuf/)** | Сгенерированные спецификации Protobuf Steam (`protobuf/steam`) и пользовательские структуры (`protobuf/custom`). |
...
| **[behavior/processor](behavior/processor/)** | Менеджер жизненного цикла транзакции (*Проверка $\rightarrow$ Решение $\rightarrow$ Действие $\rightarrow$ Уведомление*). |
...
| Пакет | Описание |
| :--- | :--- |
| **[behavior](behavior/)** | Сценарии автоматического поведения ботов (эмуляция достижений, проверка сессий, процессор сделок, авто-принятие подтверждений Guard). |
| **[command](command/)** | Регистрация CLI-команд, валидация типов на основе рефлексии и их выполнение. |
| **[bbcode](bbcode/)** | Быстрый парсер и рендер BBCode для чата и сообщества Steam. |
| **[storage](storage/)** | Провайдер постоянного хранения данных с адаптерами zero-allocation JSON (`jsonfile`) и памяти (`memory`). |

## 📐 Архитектура и принципы проектирования

Пакеты G-man спроектированы в соответствии с ключевыми практиками языка Go:

1. **Zero-Allocation на горячих путях:** Использование `foundation/codec/json.UnmarshalNoCopy` и пула кадров `aoni` для максимальной пропускной способности и минимального потребления памяти.
2. **Изолированное тестирование (Mockability):** Структуры зависят от лаконичных интерфейсов (таких как `transport.Doer` или `storage.Provider`), а не от конкретных реализаций, что упрощает написание модульных тестов.
