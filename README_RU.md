...
Архитектура G-MAN спроектирована для максимальной экономии ресурсов, обеспечивая сверхнизкий след в оперативной памяти:
- **Ядро бота:** Требует всего **~4.5 МБ** в активной куче (включая шину событий, фоновые модули и менеджеры сделок).
- **Схема TF2:** Удерживает полную, проиндексированную для O(1)-поиска схему предметов всего в **~10 МБ** кучи, что обеспечивает суммарное физическое потребление процесса на уровне **~25 МБ RSS** в продакшене.
- **Встроенное профилирование:** Вы можете запустить пошаговый замер потребления памяти и сгенерировать pprof-профиль аллокаций следующей командой:
  ```shell
