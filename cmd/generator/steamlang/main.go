...
var (
	...
	apiUrl     = flag.String("url", "https://api.github.com/repos/SteamRE/SteamKit/contents/Resources/SteamLanguage", "GitHub API URL for SteamLanguage files")
)
...
func generateEnumsFile(enums []*EnumDef, target, pkg string) {
	...
	for _, enum := range enums {
		...
		for _, val := range enum.Values {
			...
			comment := ""
			if val.Comment != "" { comment = " // " + strings.TrimSpace(val.Comment) }
		...
		for _, v := range enum.Values {
			if strings.Contains(v.Value, "|") { continue }
